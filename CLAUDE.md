...
```bash
make sim                # Full simulation flow (compile + elaborate + run, ~9148 tests)
...
- The CU runs PC `0..CU_PC_END` to completion (`CU_PC_END` at APB 0x3C, default = 15). Set `CU_PC_END = N` and `LOOP_END = N` together for a tight N+1-slot loop without wrap-up overhead. Tests verified by Suite PCE (`tb_suite_pc_end.svh`). Older versions of this doc claimed "always runs 0..15" — that was true only for the default; the actual loop bound is software-programmable.
...
5-layer modular TB with 27+ test suites, ~9148 tests + UVM-inspired transaction layer:
