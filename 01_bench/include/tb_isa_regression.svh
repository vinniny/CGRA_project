        ...
        for (op_idx = 1; op_idx <= 20; op_idx++) begin
            ...
            for (i = 0; i < 500; i++) begin
                ...
                case (op_idx)
                    ...
                    //
                    // FIX (resolved): CU_TIMEOUT=2 (set at line ~172) limits execution
                    // to ~1 MAC cycle, preventing multi-accumulation. ACC_CLR runs
                    // first to zero the accumulator, then MAC executes once.
                    // The golden model below (1 × a*b) matches this behavior.
                    4: begin
                ...
                if (op_idx == 4) begin
                    ...
                    run_cgra(2);
                    apb_write(ADDR_CU_TIMEOUT, 32'd3);  // 3 wall-clock cycles: 1 for _r, 1 for _r2, 1 for ALU
                end
