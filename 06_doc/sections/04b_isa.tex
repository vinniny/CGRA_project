...
The CGRA operates on a 64-bit Very Long Instruction Word (VLIW) architecture. Each Processing Element (PE) executes one instruction per clock cycle, defined by a 64-bit configuration frame loaded into its local context memory. The ISA supports 19 distinct operations ranging from standard arithmetic to specialized neuromorphic functions.
...
The ALU supports 19 operations categorized into arithmetic, logic, flow control, and neuromorphic updates. All operations execute in a single cycle.
...
3 & MUL & A $\times$ B (32-bit) & 13 & LOAD\_SPM & RF $\leftarrow$ SPM[A] \\
