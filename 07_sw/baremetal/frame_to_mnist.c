...
/* Generic variant: block-average + luma + invert into an ow x oh grid.
 * Same pipeline as downsample_roi_to_mnist but with caller-set grid. */
void downsample_roi_to_grid(const uint8_t *fb, hdmi_roi_t roi,
                            uint8_t *out, uint32_t ow, uint32_t oh)
{
    const uint32_t bw = roi.w / ow;
    const uint32_t bh = roi.h / oh;
    const uint32_t block_pix = (bw == 0u || bh == 0u) ? 1u : (bw * bh);
